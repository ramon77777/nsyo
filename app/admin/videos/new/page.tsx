...
async function upload(formData: FormData): Promise<void> {
  ...
  if (!res.ok) redirect(`/admin/videos/new?error=${encodeURIComponent(res.message)}`);
        ...
        <div className="mb-8 flex flex-wrap items-end justify-between gap-4">
          <div>
            ...
            <p className="mt-2 max-w-2xl text-sm text-slate-600">
              Upload direct dans Supabase Storage (bucket <span className="font-mono">media</span>).
            </p>
          </div>
        ...
        <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
          <form action={upload} className="grid gap-4">
            ...
            <div className="grid gap-4 md:grid-cols-2">
              <div>
                <div className="text-xs font-semibold text-slate-700">Fichier vidéo (MP4 recommandé)</div>
                <input
                  name="video_file"
                  type="file"
                  accept="video/*"
              ...
              <div>
                <div className="text-xs font-semibold text-slate-700">Miniature (optionnel)</div>
                <input
                  name="thumbnail_file"
                  type="file"
                  accept="image/*"
